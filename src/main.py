...
def get_developerinfo(dev_id):
    url = f"{GPLAY_BASE_URL}/api/developers/{dev_id}"
    response = requests.get(url)
    return response.json() if response.status_code == 200 else None

def get_applist_by_developer(dev_id):
    dev_apps = get_developerinfo(dev_id=dev_id)
    app_list = []
    for app in dev_apps['apps']:
...
def save_dev_info(dev_id: str) -> None:
    """
    ...
    """
    logging.log(logging.DEBUG, 'start of save_app_review_criterias')
    devinfo_filename = f"{PLAYDATA_PATH}DevInfo_{dev_id}.json"
    data = get_developerinfo(dev_id)
    with open(devinfo_filename, 'w') as file:
        json.dump(data, file, indent=4)
